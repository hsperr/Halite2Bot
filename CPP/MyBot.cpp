...
float mexican_hat(float att, float G, float G2, float distance) {
    //return att*2/(std::sqrt(3*G)*std::pow(3.141, 0.25))*(1-std::pow(distance/G, 2))*std::exp(-std::pow(distance, 2)/(2*std::pow(G, 2)));
    return att*std::exp(-G*distance*distance)-std::exp(-G2*distance*distance);
}

int main( int argc, char *argv[] ) {
    const hlt::Metadata metadata = hlt::initialize("Vectorian.mex");
    ...
    float weights[7] = { 0.0133615, 0.0133615, 0.00669743, 0.00669743, 2.00591877, 1, 1 };
    ...
    if (argc>1) {
        ...
        weights[5] = atof(argv[6]);
        weights[6] = atof(argv[7]);
    } 
