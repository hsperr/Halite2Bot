...
namespace hlt {
    struct Location {
        ...
        Location normalize() const {
            float length = std::sqrt(pos_x*pos_x+pos_y*pos_y);
            return {pos_x/length, pos_y/length};
        }

        Location multiply(float factor) const {
            return {pos_x*factor, pos_y*factor};
        }

        Location minus(float scalar) const {
            return {pos_x-scalar, pos_y-scalar};
        }

        Location plus(float scalar) const {
            return {pos_x+scalar, pos_y+scalar};
        }

        Location plus(const Location& location) const {
