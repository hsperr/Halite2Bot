...
botname = "TheDorian.Vec.V2"+sys.argv[1]
...
while True:
    try:
        ...
        for ship in my_ships:
            ...
            if closest_planet and ship.can_dock(closest_planet) and (not closest_planet.is_owned() or (closest_planet.owner == game_map.get_me() and not closest_planet.is_full())) and not (closest_ship_distance and closest_ship_distance < 10):
            ...
            if closest_ship:
                ...
                closest_point_to_ship = ship.closest_point_to(closest_ship)
                speed = min(speed, ship.calculate_distance_between(closest_point_to_ship))

            if closest_planet:
                #angle = ship.calculate_angle_between(closest_planet)
                #if abs(angle-want_angle)<10:
                closest_point_to_planet = ship.closest_point_to(closest_planet)
                speed = min(speed, ship.calculate_distance_between(closest_point_to_planet))
