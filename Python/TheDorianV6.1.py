...
def attack(ship, game_map, player=None, force_docked=False):
    ...
    entities_by_distance = OrderedDict(sorted(game_map.nearby_entities_by_distance(ship).items(), key=lambda x: x[0]))
    player_ship_distance, closest_enemy_ship = get_closest_enemy_ship(entities_by_distance, game_map.get_me().all_ships(), player=player, force_docked=~force_docked)
    player2_ship_distance, closest2_enemy_ship = get_closest_enemy_ship(entities_by_distance, game_map.get_me().all_ships())

    if player_ship_distance > player2_ship_distance*1.2:
        player_ship_distance, closest_enemy_ship = player2_ship_distance, closest2_enemy_ship 

    if target and player_ship_distance * 1.2 > ship.calculate_distance_between(target):
        ...
        if ticks == 1:
            ...
            for ship in my_ships:
                ...
                if ship.id in job2ids[ATTACKER]:
                    ...
                    if best_economy or ship.id % 6 == 0:
                        if best_economy or living_players==2:
                            command = attack(ship, game_map)
                        else:
                            command = attack(ship, game_map, player=player_with_best_economy, force_docked=best_economy)
                        if command:
