...
import sys
#0.02026674  0.02026674  0.01026701  4.99986636  0.01099974
#1.0240722   0.00790304  0.00463238  1.41492908  1.84394185
#0.00492863  0.00492863  0.00734183  3.41080449  0.3458935
biases = {
        "my_planet": (1, 0.10137867),
        "empty_planet": (1, 0.01473224),
        "enemy_planet": (1, 0.0094571),
        "my_ship": (-1, 2.908622),
        "enemy_ship": (1, 0.45789467)
}
botname = "Vectorian.fixmex"
...
def obstacles_between(ship, target, entities_by_distance, ignore=(), fudge=0.1):
    """
...
def navigate(ship, target, game_map, speed, entities_by_distance, avoid_obstacles=True, max_corrections=90, angular_step=5):
        """
        ...
        if avoid_obstacles:
            obstacles = obstacles_between(ship, target, entities_by_distance)
            if obstacles:
    ...
    try:
        ...
        for ship in my_ships:
            ...
            for entity in game_map.all_planets()+game_map.all_ships():
                ...
                global_x += contrib_x
                global_y += contrib_y
            times["contribs"].append(time.time()-t2)
...
#           command_queue.append(ship.thrust(speed, angle))
            ...
            t1 = time.time()
            command = navigate(ship, target, game_map, speed, entities_by_distance)
            times["navigate"].append(time.time()-t1)
            if command:
        ...
        #    logging.info(" {} tick: {} took: {}, total: {}, avg: {}".format(k, ticks, vals[-1], sum(vals), np.mean(vals)))
        # TURN END
    except Exception as e:
