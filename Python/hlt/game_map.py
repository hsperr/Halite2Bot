...
class Map:
    ...
    def __init__(self, my_id, width, height):
        ...
        self._players = {}
        self._planets = {}
        self._possible_obstacles = defaultdict(list)
    ...
    def all_ships(self):
        return [ship for player in self.all_players() for ship in player.all_ships() ]

    def all_enemy_ships(self):
        return [ship for player in self.all_players() for ship in player.all_ships() if not player == self.get_me() ]

    def all_own_ships(self):
    ...
    def all_empty_planets(self):
        return [x for x in self.all_planets() if x.owner is None]
    ...
    def _parse(self, map_string):
        ...
        self._possible_obstacles = defaultdict(list)
        for celestial_object in self.all_planets() + self._all_ships():
            self._possible_obstacles[(int(celestial_object.x/10), int(celestial_object.y/10))].append(celestial_object)
    ...
    def obstacles_between(self, ship, target, ignore=(), custom_targets=[]):
        """
        ...
        for foreign_entity in entities + custom_targets:
