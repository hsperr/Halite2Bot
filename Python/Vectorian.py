...
def obstacles_between(ship, target, ignore=(), fudge=0.1):
    ...
    """
    obstacles = []
    entities = game_map._possible_obstacles[(int(ship.x/10), int(ship.y/10))] + game_map._possible_obstacles[(int(target.x/10), int(target.y/10))]
    logging.info("{} {} {}".format(ship.id,(int(ship.x/10), int(ship.y/10)), (int(target.x/10), int(target.y/10))))
