...
def attack(ship, game_map):
    ...
    if not target:
        entities_by_distance = OrderedDict(sorted(game_map.nearby_entities_by_distance(ship).items(), key=lambda x: x[0]))
        player_ship_distance, closest_enemy_ship = get_closest_enemy_ship(entities_by_distance, game_map.get_me().all_ships())
    else:
        closest_enemy_ship = target
        ...
        if ticks == 1:
            ...
            for ship in my_ships:
                ...
                if ship.id in job2ids[ATTACKER]:
                    ...
                    percentage_miners = (not len(job2ids[MINER])/max(1, len(job2ids[MINER])+len(job2ids[ATTACKER])) < 0.5)
                    best_economy = len(job2ids[MINER]) >= max(5, max_other_players_mining+1)
                    should_not_be_miner = not ship.id % 5 == 0
                    logging.info("NumMiners: {} Conditions: {} {} {}".format(len(job2ids[MINER]), percentage_miners, best_economy, should_not_be_miner))
                    logging.info("Traveling Miners: {}".format(sum(1 for x in job2ids[MINER] if game_map.get_me().get_ship(x).docking_status == ship.DockingStatus.DOCKED)))
                    if percentage_miners and best_economy and should_not_be_miner:
                        command = attack(ship, game_map)
                        if command:
                            command_queue.append(command)
                    else:
