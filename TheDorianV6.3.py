...
def attack(ship, game_map, player=None, force_docked=False):
    ...
    target_pos = ship.closest_point_to(target_ship)

    speed=int(hlt.constants.MAX_SPEED)
#   if (ship.id in ship2health and ship.health < ship2health[ship.id]) or (ship.health < target_ship.health and target_distance < 10):
#       target_pos.x *= -1
#       target_pos.y *= -1

    for myship in game_map.get_me().all_ships():
        if myship.id == ship.id:
            dist = ship.calculate_distance_between(myship)
            contrib = -1 * math.exp(-dist**2)
            
            target_pos.x += contrib*(myship.x-ship.x)
            target_pos.y += contrib*(myship.y-ship.y)
...
#                           break
        ...
        ship2health = dict((x.id, x.health) for x in my_ships)
